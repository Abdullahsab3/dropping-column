...
(library
 ...
 (export new open delete!
         ...
         delete-where! print-table print alter-table-drop-column!)
 ...
 ;; toegevoegd
 (define (delete-index-from-indexes dbse indx)
   (define name (btree:name indx))
   (define idxs (indexes dbse))
   (tbl:set-current-to-first! idxs)
   (let find-index
     ((tuple (tbl:peek idxs)))
     (let ((tble-name (cadr tuple)))
       (cond ((string=? tble-name name)
              (tbl:delete! idxs (tbl:current idxs))
              (tbl:close! idxs))
             ((not (eq? (tbl:set-current-to-next! idxs) no-current))
              (find-index (tbl:peek idxs)))
             (else not-found)))))
  
 (define (alter-table-drop-column! database table attribute)
   (define idxs (indexes database))
   (let ((scma (tbl:schema table)))
     ;; als er maar 1 attribuut was in de table, wordt de table gedropt.
     (cond ((= (scma:nr-of-attributes scma) 1)
            (drop-table! database table))
           (else
            (tbl:delete-attribute! table attribute)
            ;; btrees van de oude tabel verwijderen, en nieuwe btrees maken voor de nieuwe tabel
            (for-all-indices database table (lambda (indx attr)
                                              (let ((name (btree:name indx)))
                                                (btree:drop! indx)
                                                (delete-index-from-indexes database indx)
                                                (unless (= attr attribute)
                                                  (create-index! database table name attr))))))))))