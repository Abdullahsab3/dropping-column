...
(library
 (node)
 (export new read write! record record-without record! delete!
 ...
 ;; toegevoegd
 ;; geeft de recrod van een tuple zonder het meegegeven attribuut.
 (define (record-without node slot except-attr)
   (define scma (schema node))
   (define blck (block node))
   (let loop
     ((cntr 0)
      (offs (calc-record-offset node slot)))
     (cond ((= cntr (scma:nr-of-attributes scma)) ())
           ((= cntr except-attr) (loop (+ cntr 1) (+ offs (scma:size scma cntr))))
           (else (cons ((vector-ref decoders (scma:type scma cntr))
                        blck offs (scma:size scma cntr))
                       (loop (+ cntr 1) (+ offs (scma:size scma cntr))))))))
 ;------------------------------------------------------------------------------------
